...
function loadAllFieldNotes() {
    ...
    fieldNotes.forEach((note, index) => {
        ...
        card.innerHTML = `
            <h3>${note.title}</h3>
            <div class="date">${new Date(note.date).toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
            })}</div>
...
function showActiveUsersFooter() {
    ...
    const val = localStorage.getItem('activeUsers');
    if (!activeUsersEl) {
    ...
    activeUsersEl.textContent = val && val !== '0' ? `Active Users: ${val}` : '';
}

// Run on page load
showActiveUsersFooter();
window.addEventListener('storage', showActiveUsersFooter); 